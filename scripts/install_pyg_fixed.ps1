<#
  ...
  Usage:
    # show suggested pip commands
    .\scripts\install_pyg_fixed.ps1

    # run the suggested installs (requires internet and correct permissions)
    .\scripts\install_pyg_fixed.ps1 -Install
#>
...
if ($cuda_ver -eq 'None' -or $cuda_ver -eq '' -or $cuda_ver -eq 'NoneType') {
    $tag = 'cpu'
} else {
    # e.g. '11.8' -> 'cu118'
...
foreach ($p in $packages) {
    # Use the format operator to avoid quoting issues
    $cmds += ("python -m pip install --no-cache-dir -U {0} -f {1}" -f $p, $wheel_index)
}
